...
DB_NAME = "fake_news_demo"
COL_ARTICLES = "articles"
COL_CONTENTS = "article_contents"

BASE_URL = "https://www.bbc.com/zhongwen/articles/clyd1d2g5z8o/trad"
FIRECRAWL_BASE = "http://localhost:3002"       # 不要加結尾斜線
FIRECRAWL_API_KEY = os.getenv("FIRECRAWL_API_KEY")  # 若容器需要金鑰，設這個就會自動帶 Authorization

MAX_DEPTH = 0
MAX_PAGES = 200
MIN_CONTENT_LEN = 300
BULK_SIZE = 100
TRACKING_PARAMS = {"utm_source","utm_medium","utm_campaign","utm_term","utm_content","gclid","fbclid","igshid","mc_cid","mc_eid"}
...
class LocalFirecrawl:
    """
    直接對 self-host Firecrawl 發 HTTP。
    會自動偵測可用路徑：/v2/scrape → /scrape → /v1/scrape → /api/v1/scrape
    並支援 Authorization header（若設 FIRECRAWL_API_KEY）。
    """
    ...
    def _detect_scrape_path(self) -> str:
        ...
        for path in self.CANDIDATE_SCRAPE_PATHS:
            try:
                ...
                # 200/4xx 都算「路徑存在」（例如 400 缺參數、402/429 限制等）
                if r.status_code in (200, 400, 401, 402, 403, 404, 429, 500):
                    if r.status_code != 404:
                        print(f"[Probe] Using scrape path: {path} (status={r.status_code})")
                        return path
            except Exception:
                pass
        # 若全 404，就先用第一個，實際呼叫時會再 fallback
        print("[Probe] No scrape path confirmed; default to /v2/scrape (will fallback on 404).")
        return "/v2/scrape"

    def scrape(self, url: str, wait_ms: int = 6000) -> dict:
        payload = {
            "url": url,
        ...
        raise RuntimeError("All scrape paths returned 404. Is the API port correct?")
...
for d in payload:
    try:
        ...
        content_doc = {"hash": h, "text_hash": text_hash, "markdown": md, "html": html,
                       "text": text_clean, "text_len": len(text_clean),
                       "source_url": url_norm, "updated_at": datetime.now(timezone.utc)}
        content = contents.find_one_and_update({"hash": h}, {"$set": content_doc},
                                               upsert=True, return_document=ReturnDocument.AFTER, projection={"_id": 1})
        ops.append(UpdateOne({"url": url_norm}, {"$set": {
            "url": url_norm, "title": title, "content_id": content["_id"],
            "fetched_at": datetime.now(timezone.utc), "site": f"https://{base_host}",
        }}, upsert=True))
